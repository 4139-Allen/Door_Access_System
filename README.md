...
本系统集成了 DeepSeek AI，支持通过自然语言控制门禁开门。**AI 功能是可选的**，未配置 API Key 不影响系统其他功能的正常使用。

### 📌 功能说明

- **自然语言开门**：输入"打开教学楼门禁"、"开启1号设备"等自然语言指令
- **智能设备识别**：AI 自动识别用户提到的设备名称或位置
- **上下文记忆**：支持多轮对话，记住之前提到的设备（Redis 存储，15分钟过期）
- **权限验证**：只有管理员可以使用 AI 开门功能
...
> 💡 **提示**：DeepSeek 提供一定的免费额度，个人使用通常足够。具体资费请参考官方文档。
...
- `AI_API_URL`：API 地址（默认即可）
- `AI_MODEL`：使用的模型，推荐 `deepseek-v4-flash`（快速且便宜）
- `AI_TIMEOUT`：请求超时时间（秒），默认 15 秒
- `AI_TEMPERATURE`：AI 创造性参数，0-1 之间，越低越稳定（建议 0.1）

#### 方法 2：Docker 部署时配置

在 `.env` 文件中配置好上述参数后，重启 Docker 容器：
...
启动系统后，查看日志确认 AI 功能状态：

```bash
# 查看后端日志
docker-compose logs fastapi | grep AI

# 如果配置正确，不会看到警告信息
# 如果未配置，会显示：
# ⚠️  警告: 未配置 DEEPSEEK_API_KEY，AI 智能助手功能将不可用
```

### 🎯 使用示例

配置完成后，管理员可以在前端界面的 AI 助手中尝试以下对话：
...
1. **AI 功能是可选的**：不配置 API Key 时，系统会显示警告但正常启动，其他功能不受影响
2. **仅管理员可用**：出于安全考虑，只有管理员角色可以使用 AI 开门功能
3. **费用控制**：注意监控 API 使用量，避免超额消费
4. **网络要求**：确保服务器可以访问 `api.deepseek.com`
5. **错误处理**：如果 API Key 无效或网络异常，系统会返回友好提示，不会崩溃

### 🔧 故障排查

**问题 1：提示"AI 功能未启用"**
- 检查 `.env` 文件中是否正确配置了 `DEEPSEEK_API_KEY`
- 重启服务使配置生效

**问题 2：AI 响应缓慢或超时**
- 检查网络连接是否正常
- 尝试增加 `AI_TIMEOUT` 的值（如改为 30）
- 检查 DeepSeek API 服务状态

**问题 3：API Key 无效**
- 确认 API Key 是否正确复制（不要有多余空格）
- 检查 DeepSeek 账户余额是否充足
- 访问 DeepSeek 平台验证 API Key 有效性
