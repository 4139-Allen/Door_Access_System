    ...
    <view v-if="logged">
      <view class="stats-row">
        <view class="stat-card">
          <text class="stat-num">{{ stats.user_total }}</text>
          <text class="stat-label">用户总数</text>
        </view>
        <view class="stat-card">
          <text class="stat-num">{{ stats.device_total }}</text>
          <text class="stat-label">设备总数</text>
        </view>
        <view class="stat-card">
          <text class="stat-num">{{ stats.today_log }}</text>
      ...
      <view class="section">
        ...
        <view class="quick-actions">
          ...
          <view v-if="admin" class="action-item" @click="goUsers">
            <view class="action-icon">&#128101;</view>
            <text class="action-text">用户管理</text>
          </view>
          <view v-if="admin" class="action-item" @click="goDevice">
            <view class="action-icon">&#128225;</view>
...
export default {
  data: function() {
    return {
      logged: false,
      admin: false,
      stats: { user_total: 0, device_total: 0, today_log: 0 },
      recentLogs: []
    }
  },
  onShow: function() {
