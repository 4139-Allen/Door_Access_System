...
"""
import pytest
from utils.auth import hash_password, verify_password, create_access_token, get_current_user, logout_token, require_admin
...
class TestRequireAdmin:
    ...
    def test_require_admin_with_admin_user(self, db_session):
        ...
        admin_user = User(username="admin", password=hash_password("pass"), role="admin")
    ...
    def test_require_admin_with_normal_user_raises_exception(self, db_session):
        """测试普通用户被拒绝"""
        from database.models.user import User

        normal_user = User(username="user", password=hash_password("pass"), role="user")
        ...
        assert "无管理员权限" in str(exc_info.value.detail)
