...
class TestGetDeviceList:
    ...
    def test_admin_get_all_devices(self, db_session, test_admin, test_device):
        """测试管理员获取所有设备"""
        devices = get_device_list(db_session, test_admin.id, is_admin=True)
    ...
    def test_user_get_bound_devices(self, db_session, test_user, test_device):
        """测试普通用户只获取绑定的设备"""
        # 未绑定时应该返回空列表
        devices = get_device_list(db_session, test_user.id, is_admin=False)
    ...
    def test_filter_devices_by_name(self, db_session, test_admin):
        ...
        devices = get_device_list(db_session, test_admin.id, is_admin=True, name="001")
