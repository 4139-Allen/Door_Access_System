#include "menu.h"
#include "lcd12864.h"
#include "Martix_KEY.h"
#include "password.h"
#include "string.h"
#include "delay.h"
#include "General_Module.h"
#include "usart.h"
#include "string.h"
#include "stdlib.h"
#include "stdio.h"
#include "AS608.h"

extern GeneralModule Buzzer;

extern GeneralModule Relay;

extern uint8_t Read_UID(uint8_t *UID);

uint8_t disp = 0;
uint8_t sub_disp = 0;

uint8_t select = 0;
uint8_t lcd_clear_flag = 1;

uint8_t pwd_num = 0;
uint8_t pwd[6] = {0};

uint8_t Card_ID[4] = {0x00};

uint8_t card_manage_status = 0;
int8_t selected_card_id = -1;

uint8_t fp_manage_status = 0;
int8_t selected_fp_id = -1;
uint8_t ensure = 0;

uint8_t change_pwd_status = 0;
uint8_t new_pwd[6] = {0};
uint8_t confirm_new_pwd[6] = {0};

void menu(void)
{
    if (disp == 0 && sub_disp == 0 && pwd_num == 0)
    {
        if (rx_finish_flag) // 蓝牙开锁
        {
            rx_finish_flag = 0;
            if (memcmp(Password.unlock_password, rx_buf, password_length) == 0)
            {
                lcd_clear_flag = 1;
                lcd_draw_str(2, 0, "蓝牙开锁成功");
                GeneralModule_Write(&Relay, 1);
                delay_ms(1500);
                GeneralModule_Write(&Relay, 0);
            }
            else
            {
                lcd_clear_flag = 1;
                lcd_draw_str(2, 0, "蓝牙密码错误");
                GeneralModule_Write(&Buzzer, 0);
                delay_ms(1500);
                GeneralModule_Write(&Buzzer, 1);
            }
            memset(rx_buf, 0, password_length);
        }
    }

        uint8_t UID[4];
        if (Read_UID(Card_ID) == 0) // 读卡成功
        {
            for (uint8_t i = 0; i <= 2; i++) // 查找卡
            {
                if (memcmp(Unlock_Card[i].card_id, Card_ID, 4) == 0)
                {
                    uint8_t buf[2];
                    lcd_clear_flag = 1;
                    lcd_draw_str(1, 0, "刷卡成功");
                    lcd_draw_str(2, 0, "ID:");
                    buf[0] = '0' + i;
                    buf[1] = '\0';
                    lcd_draw_str(2, 2, (const char *)buf);
                    GeneralModule_Write(&Relay, 1);
                    delay_ms(1500);
                    GeneralModule_Write(&Relay, 0);
                    break;
                }
                if (i == 2)
                {
                    lcd_clear_flag = 1;
                    lcd_draw_str(2, 0, "刷卡错误");
                    GeneralModule_Write(&Buzzer, 0);
                    delay_ms(1500);
                    GeneralModule_Write(&Buzzer, 1);
                }
            }
            memset(Card_ID, 0, sizeof(Card_ID));
        }

        if (PAin(0) == 1) // 检测到指纹
        {
                SearchResult seach;

ensure = PS_GetImage();

if(ensure == 0x00)   // 只有检测到手指才继续
{
    ensure = PS_GenChar(CharBuffer1);

    if(ensure == 0x00)
    {
        ensure = PS_HighSpeedSearch(CharBuffer1,0,20,&seach);

        if(ensure == 0x00 && seach.mathscore > 50)
        {
                        uint8_t buf[2];
                        buf[0] = '0' + seach.pageID;
                        buf[1] = '\0';
            lcd_clear_flag = 1;
            lcd_draw_str(1,0,"指纹解锁成功");
                        lcd_draw_str(2, 0, "ID:");
                        buf[0] = '0' + seach.pageID;
                        lcd_draw_str(2, 2, (const char *)buf);
                        lcd_clear_flag = 1;
            GeneralModule_Write(&Relay,1);
            delay_ms(1500);
            GeneralModule_Write(&Relay,0);
        }
        else
        {
            lcd_clear_flag = 1;
            lcd_draw_str(1,0,"指纹错误");

            GeneralModule_Write(&Buzzer,0);
            delay_ms(1000);
            GeneralModule_Write(&Buzzer,1);
        }
    }
    }
}

    if (lcd_clear_flag)
    {
        lcd_clear_flag = 0;
        lcd_draw_str(0, 0, "                                                                ");
    }

    if (disp == 0)
    {
        if (sub_disp == 0)
        {
            lcd_draw_str(0, 0, "智能电子密码锁");
            lcd_draw_str(3, 5, "Enter");
            if (pwd_num > 0) // 密码解锁
            {
                for (uint8_t i = 1; i <= pwd_num; i++)
                {
                    lcd_draw_str(1, i, "*");
                }
                if (pwd_num == 6)
                {
                    if (memcmp(pwd, Password.unlock_password, password_length) == 0) // 密码正确
                    {
                        memset(pwd, 0, password_length);
                        lcd_clear_flag = 1;
                        lcd_draw_str(2, 0, "已开锁");
                        GeneralModule_Write(&Relay, 1);
                        delay_ms(1500);
                        key_handle();
                        pwd_num = 0;
                        GeneralModule_Write(&Relay, 0);
                    }
                    else // 密码错误
                    {
                        memset(pwd, 0, password_length);
                        lcd_clear_flag = 1;
                        lcd_draw_str(2, 0, "开锁密码错误");
                        GeneralModule_Write(&Buzzer, 0);
                        delay_ms(1500);
                        key_handle();
                        pwd_num = 0;
                        GeneralModule_Write(&Buzzer, 1);
                    }
                }
            }
        }
        if (sub_disp == 1)
        {
            lcd_draw_str(0, 0, "请输入管理员密码");
            lcd_draw_str(3, 0, "Back");
            if (pwd_num > 0)
            {
                for (uint8_t i = 1; i <= pwd_num; i++)
                {
                    lcd_draw_str(1, i, "*");
                }
                if (pwd_num == 6)
                {
                    if (memcmp(pwd, Password.admin_password, password_length) == 0) // 密码正确
                    {
                        pwd_num = 0;
                        lcd_clear_flag = 1;
                        sub_disp = 0;
                        disp = 1;
                        memset(pwd, 0, password_length);
                    }
                    else // 密码错误
                    {

                        lcd_clear_flag = 1;
                        memset(pwd, 0, password_length);
                        lcd_draw_str(2, 0, "管理密码错误");
                        GeneralModule_Write(&Buzzer, 0);
                        delay_ms(1500);
                        key_handle();
                        pwd_num = 0;
                        GeneralModule_Write(&Buzzer, 1);
                    }
                }
            }
        }
    }
    if (disp == 1)
    {
        lcd_draw_str(select, 0, "*");
        lcd_draw_str(0, 2, "指纹管理");
        lcd_draw_str(1, 2, "密码管理");
        lcd_draw_str(2, 2, "ID卡管理");
        lcd_draw_str(3, 0, "Back");
        lcd_draw_str(3, 5, "Enter");
    }
    if (disp == 2) // 指纹管理
    {
        if (sub_disp == 0)
        {
            lcd_draw_str(select, 0, "*");
            lcd_draw_str(0, 1, "添加指纹");
            lcd_draw_str(1, 1, "删除指纹");
            lcd_draw_str(3, 0, "Back");
            lcd_draw_str(3, 5, "Enter");
        }
        if (sub_disp == 1)
        {
            if (fp_manage_status == 0) // 等待添加
            {
                lcd_draw_str(0, 0, "输入需要添加的");
                lcd_draw_str(1, 0, "指纹ID 0~4");
                lcd_draw_str(3, 0, "Back");
                if (selected_fp_id != -1)
                {
                    fp_manage_status = 1;
                    lcd_clear_flag = 1;
                }
            }
            if (fp_manage_status == 1) // 添加中等待第一次按指纹
            {
                lcd_draw_str(0, 0, "请按第一次指纹");
                lcd_draw_str(3, 0, "Back");
                ensure = PS_GetImage();
                if (ensure == 0x00) // 获取到指纹
                {
                    ensure = PS_GenChar(CharBuffer1); // 生成特征
                    if (ensure == 0x00)
                    {
                        fp_manage_status = 2;
                        lcd_clear_flag = 1;
                    }
                }
            }
            if (fp_manage_status == 2) // 添加中等待第二次按指纹
            {
                lcd_draw_str(0, 0, "请按第二次指纹");
                lcd_draw_str(3, 0, "Back");
                ensure = PS_GetImage();
                if (ensure == 0x00) // 获取到指纹
                {
                    ensure = PS_GenChar(CharBuffer2); // 生成特征
                    if (ensure == 0x00)
                    {
                        fp_manage_status = 3;
                        lcd_clear_flag = 1;
                    }
                }
            }
            if (fp_manage_status == 3) // 对比指纹
            {
                ensure = PS_Match();
                if (ensure == 0x00) // 模板一致
                {
                    ensure = PS_RegModel(); // 生成模板
                    if (ensure == 0x00)        // 模板生成成功
                    {
                        ensure = PS_StoreChar(CharBuffer2, selected_fp_id); // 存储指纹
                        if (ensure == 0x00)                                    // 存储指纹成功
                        {
                            lcd_clear_flag = 1;
                            lcd_draw_str(1, 0, "录入成功");
                            fp_manage_status = 0;
                            disp = 0;
                            sub_disp = 0;
                            lcd_clear_flag = 1;
                            selected_fp_id = -1;
                            delay_ms(1500);
                        }
                        else // 指纹存储失败
                        {
                            fp_manage_status = 0;
                            lcd_clear_flag = 1;
                            selected_fp_id = -1;
                            lcd_draw_str(0, 0, "指纹存储失败");
                            delay_ms(1500);
                        }
                    }
                    else // 模板生成失败
                    {
                        fp_manage_status = 0;
                        lcd_clear_flag = 1;
                        selected_fp_id = -1;
                        lcd_draw_str(0, 0, "模板生成失败");
                        delay_ms(1500);
                    }
                }
                else // 模板不一致
                {
                    fp_manage_status = 0;
                    lcd_clear_flag = 1;
                    selected_fp_id = -1;
                    lcd_draw_str(0, 0, "两次指纹不一致");
                    delay_ms(1500);
                }
            }
        }
        if (sub_disp == 2)
        {
            lcd_draw_str(0, 0, "输入需要删除的");
            lcd_draw_str(1, 0, "指纹ID 0~4");
            lcd_draw_str(3, 0, "Back");
            if (selected_fp_id != -1)
            {
                ensure = PS_DeletChar(selected_fp_id, 1);
                if (ensure == 0x00)
                {
                    lcd_draw_str(2, 0, "删除成功!");
                    selected_fp_id = -1;
                    sub_disp = 0;
                    disp = 0;
                    lcd_clear_flag = 1;
                    delay_ms(1500);
                }
                else
                {
                    lcd_draw_str(2, 0, "删除失败!");
                    selected_fp_id = -1;
                    sub_disp = 0;
                    disp = 0;
                    lcd_clear_flag = 1;
                    delay_ms(1500);
                }
            }
        }
    }
    if (disp == 3) // 密码管理
    {
        if (sub_disp == 0)
        {
            lcd_draw_str(select, 0, "*");
            lcd_draw_str(0, 1, "修改开锁密码");
            lcd_draw_str(1, 1, "修改管理员密码");
            lcd_draw_str(3, 0, "Back");
            lcd_draw_str(3, 5, "Enter");
        }
        if (sub_disp == 1) // 改开锁密码
        {
            if (change_pwd_status == 0) // 输入新密码
            {
                lcd_draw_str(0, 0, "输入新开锁密码");
                lcd_draw_str(3, 0, "Back");
                if (pwd_num > 0)
                {
                    for (uint8_t i = 1; i <= pwd_num; i++)
                    {
                        lcd_draw_str(1, i, "*");
                    }
                    if (pwd_num == 6)
                    {
                        pwd_num = 0;
                        lcd_clear_flag = 1;
                        change_pwd_status = 1;
                    }
                }
            }
            if (change_pwd_status == 1) // 确认新密码
            {
                lcd_draw_str(0, 0, "确认新开锁密码");
                lcd_draw_str(3, 0, "Back");
                if (pwd_num > 0)
                {
                    for (uint8_t i = 1; i <= pwd_num; i++)
                    {
                        lcd_draw_str(1, i, "*");
                    }
                    if (pwd_num == 6)
                    {
                        pwd_num = 0;
                        lcd_clear_flag = 1;
                        change_pwd_status = 0;
                        disp = 0;
                        sub_disp = 0;
                        if (memcmp(new_pwd, confirm_new_pwd, password_length) == 0) // 对比两次密码是否一致
                        {
                            ChangePassword(0, confirm_new_pwd);
                            lcd_draw_str(2, 0, "修改成功");
                            delay_ms(1500);
                        }
                        else
                        {
                            lcd_draw_str(2, 0, "两次密码不一致");
                            delay_ms(1500);
                        }
                        memset(new_pwd, 0, password_length);
                        memset(confirm_new_pwd, 0, password_length);
                    }
                }
            }
        }
        if (sub_disp == 2) // 改管理密码
        {
            if (change_pwd_status == 0) // 输入新密码
            {
                lcd_draw_str(0, 0, "输入新管理密码");
                lcd_draw_str(3, 0, "Back");
                if (pwd_num > 0)
                {
                    for (uint8_t i = 1; i <= pwd_num; i++)
                    {
                        lcd_draw_str(1, i, "*");
                    }
                    if (pwd_num == 6)
                    {
                        pwd_num = 0;
                        lcd_clear_flag = 1;
                        change_pwd_status = 1;
                    }
                }
            }
            if (change_pwd_status == 1) // 确认新密码
            {
                lcd_draw_str(0, 0, "确认新管理密码");
                lcd_draw_str(3, 0, "Back");
                if (pwd_num > 0)
                {
                    for (uint8_t i = 1; i <= pwd_num; i++)
                    {
                        lcd_draw_str(1, i, "*");
                    }
                    if (pwd_num == 6)
                    {
                        pwd_num = 0;
                        lcd_clear_flag = 1;
                        change_pwd_status = 0;
                        disp = 0;
                        sub_disp = 0;
                        if (memcmp(new_pwd, confirm_new_pwd, password_length) == 0) // 对比两次密码是否一致
                        {
                            ChangePassword(1, confirm_new_pwd);
                            lcd_draw_str(2, 0, "修改成功");
                            delay_ms(1500);
                        }
                        else
                        {
                            lcd_draw_str(2, 0, "两次密码不一致");
                            delay_ms(1500);
                        }
                        memset(new_pwd, 0, password_length);
                        memset(confirm_new_pwd, 0, password_length);
                    }
                }
            }
        }
    }
    if (disp == 4) // ID卡管理
    {

        if (sub_disp == 0)
        {
            lcd_draw_str(select, 0, "*");
            lcd_draw_str(0, 1, "添加卡");
            lcd_draw_str(1, 1, "删除卡");
            lcd_draw_str(3, 0, "Back");
            lcd_draw_str(3, 5, "Enter");
        }
        if (sub_disp == 1)
        {
            if (card_manage_status == 0)
            {
                lcd_draw_str(0, 0, "输入要添加的卡");
                lcd_draw_str(1, 0, "ID0~2");
                lcd_draw_str(3, 0, "Back");
            }
            if (card_manage_status == 1)
            {
                lcd_draw_str(0, 1, "请刷卡");
                lcd_draw_str(3, 0, "Back");
                if (Read_UID(Card_ID) == 0)
                {
                    AddCard(selected_card_id, Card_ID);
                    card_manage_status = 0;
                    selected_card_id = -1;
                    memset(Card_ID, 0, sizeof(Card_ID));
                    lcd_draw_str(2, 1, "添加成功");
                    delay_ms(1500);
                    lcd_clear_flag = 1;
                    sub_disp = 0;
                    disp = 0;
                }
            }
        }
        if (sub_disp == 2)
        {
            lcd_draw_str(0, 0, "输入要删除的卡");
            lcd_draw_str(1, 0, "ID0~2");
            lcd_draw_str(3, 0, "Back");
            if (selected_card_id != -1)
            {
                DelCard(selected_card_id);
                selected_card_id = -1;
                lcd_draw_str(2, 1, "删除成功");
                delay_ms(1500);
                lcd_clear_flag = 1;
                sub_disp = 0;
                disp = 0;
            }
        }
    }
}

void key_handle(void)
{
    if (disp == 0) // 主页
    {
        if (sub_disp == 0)
        {
            if (pwd_num < 6)
            {
                for (uint8_t i = 0; i <= 9; i++)
                {
                    if (key_value[i] == 1)
                    {
                        key_value[i] = 0;
                        lcd_clear_flag = 1;
                        pwd_num++;
                        pwd[pwd_num - 1] = '0' + i;
                    }
                }
            }
            else
            {
                for (uint8_t i = 0; i <= 9; i++)
                {
                    if (key_value[i] == 1)
                    {
                        key_value[i] = 0;
                        pwd_num = 6;
                    }
                }
            }
            if (key_value[14] == 1) // Back
            {
                key_value[14] = 0;
                lcd_clear_flag = 1;
                pwd_num = 0;
                memset(pwd, 0, sizeof(password_length));
            }
            if (key_value[15] == 1)
            {
                key_value[15] = 0;
                lcd_clear_flag = 1;
                sub_disp = 1;
                pwd_num = 0;
                memset(pwd, 0, sizeof(password_length));
            }
        }
        if (sub_disp == 1)
        {
            if (pwd_num < 6)
            {
                for (uint8_t i = 0; i <= 9; i++)
                {
                    if (key_value[i] == 1)
                    {
                        key_value[i] = 0;
                        lcd_clear_flag = 1;
                        pwd_num++;
                        pwd[pwd_num - 1] = '0' + i;
                    }
                }
            }
            else
            {
                for (uint8_t i = 0; i <= 9; i++)
                {
                    if (key_value[i] == 1)
                    {
                        key_value[i] = 0;
                    }
                }
            }
            if (key_value[14] == 1) // Back
            {
                key_value[14] = 0;
                lcd_clear_flag = 1;
                if (pwd_num > 0)
                {
                    pwd_num = 0;
                    memset(pwd, 0, sizeof(password_length));
                }
                else
                {
                    sub_disp = 0;
                }
            }
        }
    }
    if (disp == 1) // 选择管理
    {
        if (key_value[13] == 1) // 下
        {
            key_value[13] = 0;
            lcd_clear_flag = 1;
            if (select < 2)
            {
                select++;
            }
        }
        if (key_value[12] == 1) // 上
        {
            key_value[12] = 0;
            lcd_clear_flag = 1;
            if (select > 0)
            {
                select--;
            }
        }
        if (key_value[14] == 1) // Back
        {
            key_value[14] = 0;
            lcd_clear_flag = 1;
            disp = 0;
            select = 0;
        }
        if (key_value[15] == 1) // Enter
        {
            key_value[15] = 0;
            lcd_clear_flag = 1;
            disp = select + 2; // 显示选中菜单
            select = 0;
        }
    }
    if (disp == 2) // 指纹管理
    {
        if (sub_disp == 0) // 指纹管理主页面
        {
            if (key_value[13] == 1) // 下
            {
                key_value[13] = 0;
                lcd_clear_flag = 1;
                if (select < 1)
                {
                    select++;
                }
            }
            if (key_value[12] == 1) // 上
            {
                key_value[12] = 0;
                lcd_clear_flag = 1;
                if (select > 0)
                {
                    select--;
                }
            }
            if (key_value[14] == 1) // Back
            {
                key_value[14] = 0;
                lcd_clear_flag = 1;
                disp = 1;
                select = 0;
            }
            if (key_value[15] == 1) // Enter
            {
                key_value[15] = 0;
                lcd_clear_flag = 1;
                sub_disp = select + 1; // 显示选中菜单
                select = 0;
            }
        }
        if (sub_disp == 1) // 添加指纹
        {
            if (fp_manage_status == 0)
            {
                for (uint8_t i = 0; i <= 4; i++)
                {
                    if (key_value[i] == 1)
                    {
                        key_value[i] = 0;
                        lcd_clear_flag = 1;
                        selected_fp_id = i;
                    }
                }
            }
            if (key_value[14] == 1) // Back
            {
                key_value[14] = 0;
                lcd_clear_flag = 1;
                disp = 2;
                sub_disp = 0;
                select = 0;
                fp_manage_status = 0;
                selected_fp_id = -1;
            }
        }
        if (sub_disp == 2) // 删除指纹
        {
            if (selected_fp_id == -1)
            {
                for (uint8_t i = 0; i <= 4; i++)
                {
                    if (key_value[i] == 1)
                    {
                        key_value[i] = 0;
                        lcd_clear_flag = 1;
                        selected_fp_id = i;
                    }
                }
            }
            if (key_value[14] == 1) // Back
            {
                key_value[14] = 0;
                lcd_clear_flag = 1;
                disp = 2;
                sub_disp = 0;
                select = 0;
                selected_fp_id = -1;
            }
        }
    }
    if (disp == 3) // 密码管理
    {
        if (sub_disp == 0) // 主页
        {
            if (key_value[13] == 1) // 下
            {
                key_value[13] = 0;
                lcd_clear_flag = 1;
                if (select < 1)
                {
                    select++;
                }
            }
            if (key_value[12] == 1) // 上
            {
                key_value[12] = 0;
                lcd_clear_flag = 1;
                if (select > 0)
                {
                    select--;
                }
            }
            if (key_value[14] == 1) // Back
            {
                key_value[14] = 0;
                lcd_clear_flag = 1;
                disp = 1;
                select = 0;
                pwd_num = 0;
                change_pwd_status = 0;
                memset(new_pwd, 0, password_length);
                memset(confirm_new_pwd, 0, password_length);
            }
            if (key_value[15] == 1) // Enter
            {
                key_value[15] = 0;
                lcd_clear_flag = 1;
                sub_disp = select + 1; // 显示选中菜单
                select = 0;
                pwd_num = 0;
                change_pwd_status = 0;
                memset(new_pwd, 0, password_length);
                memset(confirm_new_pwd, 0, password_length);
            }
        }
        if (sub_disp == 1 || sub_disp == 2) // 改开锁或者管理密码
        {
            if (change_pwd_status == 0)
            {
                if (pwd_num < 6)
                {
                    for (uint8_t i = 0; i <= 9; i++)
                    {
                        if (key_value[i] == 1)
                        {
                            key_value[i] = 0;
                            lcd_clear_flag = 1;
                            pwd_num++;
                            new_pwd[pwd_num - 1] = '0' + i;
                        }
                    }
                }
                else
                {
                    for (uint8_t i = 0; i <= 9; i++)
                    {
                        if (key_value[i] == 1)
                        {
                            key_value[i] = 0;
                        }
                    }
                }
            }

            if (change_pwd_status == 1)
            {
                if (pwd_num < 6)
                {
                    for (uint8_t i = 0; i <= 9; i++)
                    {
                        if (key_value[i] == 1)
                        {
                            key_value[i] = 0;
                            lcd_clear_flag = 1;
                            pwd_num++;
                            confirm_new_pwd[pwd_num - 1] = '0' + i;
                        }
                    }
                }
                else
                {
                    for (uint8_t i = 0; i <= 9; i++)
                    {
                        if (key_value[i] == 1)
                        {
                            key_value[i] = 0;
                        }
                    }
                }
            }

            if (key_value[14] == 1) // Back
            {
                key_value[14] = 0;
                lcd_clear_flag = 1;
                disp = 3;
                sub_disp = 0;
                select = 0;
                pwd_num = 0;
                change_pwd_status = 0;
                memset(new_pwd, 0, password_length);
                memset(confirm_new_pwd, 0, password_length);
            }
        }
    }
    if (disp == 4) // ID卡管理
    {
        if (sub_disp == 0)
        {
            if (key_value[13] == 1) // 下
            {
                key_value[13] = 0;
                lcd_clear_flag = 1;
                if (select < 1)
                {
                    select++;
                }
            }
            if (key_value[12] == 1) // 上
            {
                key_value[12] = 0;
                lcd_clear_flag = 1;
                if (select > 0)
                {
                    select--;
                }
            }
            if (key_value[14] == 1) // Back
            {
                key_value[14] = 0;
                lcd_clear_flag = 1;
                disp = 1;
                select = 0;
            }
            if (key_value[15] == 1) // Enter
            {
                key_value[15] = 0;
                lcd_clear_flag = 1;
                sub_disp = select + 1; // 显示选中菜单
                select = 0;
            }
        }
        if (sub_disp == 1)
        {
            if (key_value[0] == 1) // 0
            {
                key_value[0] = 0;
                lcd_clear_flag = 1;
                card_manage_status = 1;
                selected_card_id = 0;
            }
            if (key_value[1] == 1) // 1
            {
                key_value[1] = 0;
                lcd_clear_flag = 1;
                card_manage_status = 1;
                selected_card_id = 1;
            }
            if (key_value[2] == 1) // 2
            {
                key_value[2] = 0;
                lcd_clear_flag = 1;
                card_manage_status = 1;
                selected_card_id = 2;
            }
            if (key_value[14] == 1) // Back
            {
                key_value[14] = 0;
                lcd_clear_flag = 1;
                if (card_manage_status == 0)
                {
                    sub_disp = 0;
                    select = 0;
                    card_manage_status = 0;
                    selected_card_id = -1;
                }
                if (card_manage_status == 1)
                {
                    select = 0;
                    card_manage_status = 0;
                }
            }
        }
        if (sub_disp == 2)
        {
            if (key_value[0] == 1) // 0
            {
                key_value[0] = 0;
                lcd_clear_flag = 1;
                selected_card_id = 0;
            }
            if (key_value[1] == 1) // 1
            {
                key_value[1] = 0;
                lcd_clear_flag = 1;
                selected_card_id = 1;
            }
            if (key_value[2] == 1) // 2
            {
                key_value[2] = 0;
                lcd_clear_flag = 1;
                selected_card_id = 2;
            }
            if (key_value[14] == 1) // Back
            {
                key_value[14] = 0;
                lcd_clear_flag = 1;
                sub_disp = 0;
                select = 0;
                card_manage_status = 0;
                selected_card_id = -1;
            }
        }
    }
}
